...
public interface MedicalReportRepository extends JpaRepository<MedicalReport, Integer> {
    ...
    @Query("SELECT mr FROM MedicalReport mr WHERE LOWER(mr.reportType) LIKE LOWER(CONCAT('%', :reportType, '%')) ORDER BY mr.reportDate DESC")
    List<MedicalReport> findByReportTypeContainingIgnoreCase(@Param("reportType") String reportType);
    ...
    @Query("SELECT mr FROM MedicalReport mr WHERE mr.id = :id")
    Optional<MedicalReport> findById(@Param("id") Integer id);
    
    // Check if medical report exists by ID
    @Query("SELECT CASE WHEN COUNT(mr) > 0 THEN true ELSE false END FROM MedicalReport mr WHERE mr.id = :id")
    boolean existsById(@Param("id") Integer id);
    
    // Count total medical reports
    @Query("SELECT COUNT(mr) FROM MedicalReport mr")
    long count();
    
    // Delete medical report by ID
    @Modifying
    @Transactional
    @Query("DELETE FROM MedicalReport mr WHERE mr.id = :id")
    ...
    @Modifying
    @Transactional
    @Query("UPDATE MedicalReport mr SET mr.reportType = :reportType, mr.documentPath = :documentPath, mr.patient.id = :patientId WHERE mr.id = :id")
