...
@Repository
public interface Admin_PatientRepository extends JpaRepository<Admin_Patient, Integer> {
    ...
    // Add method to find patient by email
    @Query("SELECT p FROM Admin_Patient p WHERE p.email = :email")
    Admin_Patient findByEmail(@Param("email") String email);
